    ...
    class CacheNetObserver<T> extends Subscriber<T> {
        ...
        public void onCompleted() {
            if (finalStepIndex == observableWrapper.getIndex()) {
                if (debug) Log.e(TAG, "onCompleted stepIndex:" + observableWrapper.getIndex());
                try {
                ...
                if (debug)
                    Log.e(TAG, "onCompleted stepIndex:" + observableWrapper.getIndex() + " still not completed");
                overCount.addAndGet(1);
            }
        ...
        public void onError(Throwable e) {
            if (debug) {
                Log.e(TAG, "onError stepIndex:" + observableWrapper.getIndex());
            }
            if (finalStepIndex == observableWrapper.getIndex()) {
...
//                }
        ...
        public void onNext(T o) {
            if (debug)
                Log.e(TAG, "stepIndex :" + stepIndex + " observableWrapper.getIndex():" + observableWrapper.getIndex() + " o:" + o);
            observableWrapper.setData(o);
            if (stepIndex < observableWrapper.getIndex()) {
                stepIndex = observableWrapper.getIndex();
                if (debug)
                    Log.e(TAG, "stepIndex :" + stepIndex + " observableWrapper.getIndex():" + observableWrapper.getIndex());
                logThread("");
                if (debug)
                    Log.e(TAG, " check subscriber :" + subscriber + " isUnsubscribed:" + subscriber.isUnsubscribed());
                if (subscriber != null && !subscriber.isUnsubscribed()) {
    ...
    public void logThread(String tag) {
        if (debug) Log.e(TAG, tag + " : " + Thread.currentThread().getName());
    }
