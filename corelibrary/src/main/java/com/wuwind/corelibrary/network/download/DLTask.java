...
import static com.wuwind.corelibrary.network.download.DLCons.Base.DEFAULT_TIMEOUT;
import static com.wuwind.corelibrary.network.download.DLCons.Base.LENGTH_PER_THREAD;
import static com.wuwind.corelibrary.network.download.DLCons.Base.MAX_REDIRECTS;
import static com.wuwind.corelibrary.network.download.DLCons.Code.HTTP_MOVED_PERM;
import static com.wuwind.corelibrary.network.download.DLCons.Code.HTTP_MOVED_TEMP;
import static com.wuwind.corelibrary.network.download.DLCons.Code.HTTP_NOT_MODIFIED;
import static com.wuwind.corelibrary.network.download.DLCons.Code.HTTP_OK;
import static com.wuwind.corelibrary.network.download.DLCons.Code.HTTP_PARTIAL;
import static com.wuwind.corelibrary.network.download.DLCons.Code.HTTP_SEE_OTHER;
import static com.wuwind.corelibrary.network.download.DLCons.Code.HTTP_TEMP_REDIRECT;
import static com.wuwind.corelibrary.network.download.DLCons.DEBUG;
import static com.wuwind.corelibrary.network.download.DLError.ERROR_OPEN_CONNECT;
...
class DLTask implements Runnable, IDLThreadListener {
    ...
    DLTask(Context context, DLInfo info) {
        ...
        if (DEBUG) Log.d(TAG, "All the threads was stopped.");
    }
    ...
    public synchronized void onProgress(int progress) {
        ...
        if (currentTime - lastTime > 1000) {
            if (DEBUG) Log.d(TAG, "totalProgress:" + totalProgress);
    ...
    public synchronized void onFinish(DLThreadInfo threadInfo) {
        ...
        if (DEBUG) Log.d(TAG, "Thread size " + info.threads.size());
        if (info.threads.isEmpty()) {
            if (DEBUG) Log.d(TAG, "Task was finished.");
            DLManager.getInstance(context).removeDLTask(info.baseUrl);
            DLDBManager.getInstance(context).deleteTaskInfo(info.baseUrl);
            if (DEBUG) Log.d(TAG, "deleteTaskInfo.");
            if (info.hasListener) {
                info.listener.onProgress(info.totalBytes);
                info.listener.onFinish(info.file);
            }
    ...
    @Override
    public void onError(DLThreadInfo threadInfo) {
        DLManager.getInstance(context).dlStop(threadInfo.baseUrl);
    }
    ...
    public void run() {
        ...
        while (info.redirect < MAX_REDIRECTS) {
            if (DEBUG) Log.d(TAG, "redirect:" + info.redirect);
    ...
    private void dlInit(HttpURLConnection conn, int code) throws Exception {
        ...
        if (info.file.exists() && info.file.length() == info.totalBytes) {
            if (DEBUG) Log.d(TAG, "The file which we want to download was already here.");
            if (!info.loadAnyway) {
                onFinish(null);
                return;
            }
        ...
        switch (code) {
            ...
            case HTTP_PARTIAL:
                ...
                if (info.isResume) {
                    if (DEBUG) Log.d(TAG, "isResume");
                    for (DLThreadInfo threadInfo : info.threads) {
                        if (DEBUG) Log.d(TAG, "threadInfo");
    ...
    private void readResponseHeaders(HttpURLConnection conn) {
        ...
        if (DEBUG) {
            Log.d(TAG, "transferEncoding:" + transferEncoding);
            Log.d(TAG, "disposition:" + info.disposition);
            Log.d(TAG, "location:" + info.location);
        }
        if (TextUtils.isEmpty(transferEncoding)) {
            try {
                info.totalBytes = Integer.parseInt(conn.getHeaderField("Content-Length"));
                if (DEBUG) Log.d(TAG, "totalBytes" + info.totalBytes);
        ...
        if (info.totalBytes == -1 && (TextUtils.isEmpty(transferEncoding) || !transferEncoding.equalsIgnoreCase("chunked")))
